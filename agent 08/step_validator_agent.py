...
def _truncate(data: str, max_chars: int = 80000) -> str:
    ...
    if len(data) <= max_chars:
        return data
    print(f"[VALIDATOR] WARNING: input truncated from {len(data)} to {max_chars} chars — LLM will see partial data")
...
async def validate_step(
    ...
    # Parse response — strip markdown fences, then fix trailing commas which
    # LLMs occasionally produce (e.g. the last key in an object or array).
    import re as _re
    clean = raw_response.replace("```json", "").replace("```", "").strip()
    # Remove trailing commas before } or ] — handles LLM JSON formatting slips
