...
class ReviewerAPIError(Exception):
    """Raised when the LLM API returns a hard error (rate limit, quota exceeded, auth)
    that makes it impossible to complete the document review. Propagates to the
    /review endpoint which aborts with a clear error response."""
    pass
...
def _llm_call(prompt: str) -> Any:
    ...
    for attempt in range(_LLM_RETRIES):
        try:
            ...
            err_str = str(e)
            # 429 rate-limit / quota errors: retrying immediately won't help.
            # Surface immediately so the review endpoint can abort and report clearly.
            if "429" in err_str or "quota" in err_str.lower() or "rate limit" in err_str.lower():
                raise ReviewerAPIError(
                    f"OpenAI API rate limit or quota error — document review cannot proceed: {e}"
                ) from e
            last_exc = e
...
def check_filename_match(filename: str, search_context: str) -> dict:
    ...
    Raises ReviewerAPIError on rate-limit/quota failures — caller must handle."""
    prompt = (
    ...
    except ReviewerAPIError:
        raise  # propagate — /review endpoint will abort with error response
    except Exception as e:
...
def review_documents(request: ReviewRequest):
    ...
    except ReviewerAPIError as api_err:
        # LLM API hard failure (rate limit / quota exhausted) — abort the entire review.
        # Return HTTP 503 so the orchestrator retries rather than continuing with 0 matches.
        err_msg = str(api_err)
        print(f"[Reviewer] FATAL: {err_msg}")
        raise HTTPException(
            status_code=503,
            detail=(
                f"Document review aborted — LLM API error: {err_msg}. "
                f"This is usually a rate-limit or quota issue. Retry step 1."
            ),
        )
    ...
    # Detect LLM API failure: if every file scored exactly 0.0 confidence and nothing matched,
    # the LLM calls (filename match, content match) silently returned empty/false for all files.
    # This is a clear API failure — log a prominent warning so it appears in the orchestrator log.
    if scan_results and not matched and all(r["overall_confidence"] == 0.0 for r in scan_results):
        print(
            f"[Reviewer] WARNING: ALL {len(scan_results)} file(s) returned confidence=0.0 with no matches. "
            f"This strongly indicates an LLM API failure during filename or content matching. "
            f"Retry step 1 — do not proceed with downstream steps on this result."
        )
...
def _detect_page_size(filepath: str) -> dict:
    ...
    CHUNK_MAP = {"A3": 2, "A2": 4, "A1": 6, "A0": 9, "large-format": 12}
...
def _determine_processing_plan(doc: dict, scan: dict, registry: dict, categorization_rules: str = "", page_size_info: dict = None) -> dict:
    ...
    prompt = (
        ...
        f"- Requires Chunking: {req_chunk} (pre-assessed: A3 and larger pages must be split for LLM processing)\n"
        ...
        f"7. requires_chunking: true if page_size is A3, A2, A1, A0, or large-format; otherwise false\n"
        f"8. chunk_strategy: 'quadrant-split' for A3 and larger drawings, 'page-split' for multi-page text docs with oversized pages, 'none' if not chunking\n"
        f"9. estimated_chunks: Expected number of chunks per page (A3→2, A2→4, A1→6, A0→9, large-format→12, otherwise 1)\n"
