...
---------------------------------------------
A3  →  4 chunks  →  dual-split: 2 top/bottom (1×2) + 2 left/right (2×1)
...
_CHUNKS_PER_PAGE: dict[str, int] = {
    "A3": 4,   # 2×2 grid — A3 drawings need quadrant splitting for readable text resolution
    "A2": 4,
    "A1": 6,
    "A0": 9,
    "large-format": 12,
...
async def chunk_document(
    ...
    elif chunk_strategy == "dual-split":
        # Pass 1: top/bottom (1×2) — ensures bottom notes section is a dedicated chunk
        tb_labels = _region_labels(1, 2)
        for page_num, page_img in enumerate(pages, start=1):
            for tile_idx, tile in enumerate(_split_image(page_img, 1, 2)):
                fname = f"chunk_{seq:04d}.png"
                fpath = job_dir / fname
                tile.save(str(fpath), "PNG")
                w, h = tile.size
                chunks.append({
                    ...
                    "sequence":    seq,
                    "page_number": page_num,
                    "region":      tb_labels[tile_idx] if tile_idx < len(tb_labels) else f"tb-{tile_idx + 1}",
                    "grid":        "1x2",
                    "col":         1,
                    "row":         tile_idx + 1,
                    "split_pass":  1,
                ...
                seq += 1
        # Pass 2: left/right (2×1) — ensures content split by vertical centre is covered
        lr_labels = _region_labels(2, 1)
        for page_num, page_img in enumerate(pages, start=1):
            for tile_idx, tile in enumerate(_split_image(page_img, 2, 1)):
                fname = f"chunk_{seq:04d}.png"
                fpath = job_dir / fname
                tile.save(str(fpath), "PNG")
                w, h = tile.size
                chunks.append({
                    ...
                    "sequence":    seq,
                    "page_number": page_num,
                    "region":      lr_labels[tile_idx] if tile_idx < len(lr_labels) else f"lr-{tile_idx + 1}",
                    "grid":        "2x1",
                    "col":         tile_idx + 1,
                    "row":         1,
                    "split_pass":  2,
