...
Checklist when creating a new agent:
  ...
  [ ] Add a process/*.json if the agent needs process-driven configuration

Common framework modules (all in common/):
  - agent.py       create_app()         FastAPI app factory, CORS, /health
  - config.py      settings             Centralised env-var config
  - llm.py         LLMClient            Multi-provider LLM client (OpenAI/Anthropic/Gemini) with retry
  - output.py      OutputManager        Timestamped JSON file writing and FilesManifest tracking
  - models.py      FilesManifest etc.   Shared Pydantic models
  - knowledge.py   KnowledgeRegistry    Process JSON loader — routing, extraction config, validation directives
"""
...
# Optional: load process-driven configuration from process/*.json
# Remove if this agent does not use process knowledge files.
...
def _do_work(input_text: str) -> Dict[str, Any]:
    ...
    """
    # Example: query process-driven config from KnowledgeRegistry
    # route    = knowledge.route_for_category("Design Brief")   # → 4
    # model    = knowledge.model_for_process("proc-extract-design-brief-info")
    # directives = knowledge.validation_directives_for_process("proc-extract-design-brief-info")
